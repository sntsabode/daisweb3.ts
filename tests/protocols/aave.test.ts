...
describe('AaveWriter Test Suite', () => {
  beforeEach(() =>
    mock({
  ...
  it('Should call the AaveWriter function with the "ILENDINGPOOLADDRESSESPROVIDER" option', async () => {
    ...
    assert.strictEqual(
      res.Addresses[0].ContractName,
      'ILendingPoolAddressesProvider'
    )
    assert.strictEqual(
      res.Addresses[0].Address,
      Addresses.AAVE.LendingPoolAddressesProvider.MAINNET
    )

    assert.strictEqual(res.Pack, NPMPacks.AAVE.V2SDK)

    const ILendingPoolAddressesProvider = readFileSync(
      'test-dir/contracts/interfaces/Aave/ILendingPoolAddressesProvider.sol'
    ).toString()
    assert.strictEqual(
      ILendingPoolAddressesProvider,
  ...
  it('Should call the AaveWriter function with the "ILENDINGPOOL" option', async () => {
    ...
    const ILendingPool = readFileSync(
      'test-dir/contracts/interfaces/Aave/ILendingPool.sol'
    ).toString()
    assert.strictEqual(ILendingPool, Aave.Interfaces.ILendingPool(solver))

    const DataTypes = readFileSync(
      'test-dir/contracts/libraries/Aave/DataTypes.sol'
    ).toString()
    assert.strictEqual(DataTypes, Aave.Libraries.DataTypes(solver))
  })

  it('Should call the AaveWriter function with an erroneous input', async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      try {
        readFileSync(
          'test-dir/contracts/interfaces/Bancor/IBancorNetwork.sol'
        ).toString()
        readFileSync(
          'test-dir/contracts/interfaces/Bancor/IContractRegistry.sol'
        ).toString()
