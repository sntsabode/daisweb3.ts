...
describe('OneInchWriter Test Suite', () => {
  beforeEach(() =>
    mock({
  ...
  it('Should call the OneInchWriter function with the "ONESPLIT" option', async () => {
    ...
    const IOneSplit = readFileSync(
      'test-dir/contracts/interfaces/OneInch/IOneSplit.sol'
    ).toString()
    assert.strictEqual(IOneSplit, OneInch.Interfaces.IOneSplit(solver))
  })

  it('Should call the OneInchWriter function with the "ONESPLITMULTI" option', async () => {
    const res = await OneInchWriter(dir, solver, 'all', {
      protocol: 'ONEINCH',
      pack: 'ONESPLITMULTI',
      omitNpmPack: true,
    ...
    const IOneSplitMulti = readFileSync(
      'test-dir/contracts/interfaces/OneInch/IOneSplitMulti.sol'
    ).toString()
    assert.strictEqual(
      IOneSplitMulti,
  ...
  it('Should call the OneInchWriter function with an erroneous input, whilst omitting the npmPack', async () => {
