...
describe('DyDxWriter Test Suite', () => {
  beforeEach(() =>
    mock({
      'test-dir/contracts': {},
  ...
  it('Should call the DyDxWriter function with the "FLASHLOAN" option', async () => {
    ...
    const Flashloan = readFileSync(
      'test-dir/contracts/Flashloan.sol'
    ).toString()
    assert.strictEqual(Flashloan, DyDx.Libraries.FlashloanBoilerplate(solver))

    const ICallee = readFileSync(
      'test-dir/contracts/interfaces/DyDx/ICallee.sol'
    ).toString()
    assert.strictEqual(ICallee, DyDx.Interfaces.ICallee(solver))

    const ISoloMargin = readFileSync(
      'test-dir/contracts/interfaces/DyDx/ISoloMargin.sol'
    ).toString()
    assert.strictEqual(ISoloMargin, DyDx.Interfaces.ISoloMargin(solver))

    const Account = readFileSync(
      'test-dir/contracts/libraries/DyDx/Account.sol'
    ).toString()
    assert.strictEqual(Account, DyDx.Libraries.Account(solver))

    const Actions = readFileSync(
      'test-dir/contracts/libraries/DyDx/Actions.sol'
    ).toString()
    assert.strictEqual(Actions, DyDx.Libraries.Actions(solver))

    const Types = readFileSync(
      'test-dir/contracts/libraries/DyDx/Types.sol'
    ).toString()
    assert.strictEqual(Types, DyDx.Libraries.Types(solver))
  })

  it('Should call the DyDxWriter function with an erroneous input, whilst omitting the npmPack', async () => {
