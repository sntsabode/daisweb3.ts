...
describe('ProtocolFileWriter Test Suite', () => {
  ...
  it('Should call the makeBaseDirs function', async () => {
    const dirPromise = (path: string) =>
  ...
  // TODO: Refactor it
  it('Should call the ProtocolFileWriter.main method', async () => {
    ...
    const deps = await ProtocolFileWriter.instance.main(
      'test-dir',
      [
        {
          protocol: 'BANCOR',
    ...
    for (const dep of deps) assert.isNotEmpty(dep)

    const ABIs_ = fs
      .readFileSync(resolve('test-dir/lib/__abis__/abis.ts'))
      .toString()
    ...
    const Addresses_ = fs
      .readFileSync(resolve('test-dir/lib/addresses.ts'))
      .toString()
    // evil eval :0
  ...
  it('Should call the ProtocolFileWriter.main method with an all erroneous contractImports param', async () => {
    const ProtocolFileWriter02 = ProtocolFileWriter.instance.newInstance()
    const deps = await ProtocolFileWriter02.main(
      'test-dir',
      [
        {
