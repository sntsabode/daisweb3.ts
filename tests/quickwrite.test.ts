...
describe('QuickWrite Test Suite', () => {
  ...
  it('Should call the QuickWrite function with 4 parameters (calling the protocolPackAbiAndOmitNpmPack function)', async () => {
    await QuickWrite(
      ['UNISWAP', 'V2ROUTER02', 'false', 'false'],
      childWorkingDir
    )
  })
  ...
  it('Should call the QuickWrite function with 5 parameters (calling the networkParamMissing function)', async () => {
    await QuickWrite(
      ['DYDX', 'FLASHLOAN', 'false', 'omitNpmPack', '0.8.6'],
      childWorkingDir
    )
  })

  it('Should call the QuickWrite function with all parameters (calling the quickWrite function)', async () => {
    mkdirSync(join(childWorkingDir + '/contracts/interfaces/Uniswap'), {
      recursive: true
    })
    await QuickWrite(
      ['UNISWAP', 'V2ROUTER02', 'abi', 'false', '0.8.6', 'all'],
      childWorkingDir,
      childWorkingDir
    )
    ...
    const [
      ...
      readFileSync(
        resolve(childWorkingDir + '/lib/__abis__/abis/IUniswapV2Router01.json')
      ).toString(),
      readFileSync(
        resolve(childWorkingDir + '/lib/__abis__/abis/IUniswapV2Router02.json')
      ).toString(),
    ...
    assert.strictEqual(
      IUniswapV2Router01,
      Uniswap.Interfaces.IUniswapV2Router01('0.8.6')
    )
    assert.strictEqual(
      IUniswapV2Router02,
      Uniswap.Interfaces.IUniswapV2Router02('0.8.6')
    )
