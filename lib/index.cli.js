...
}).usage('daisweb3 -a').help(
).option('yes', {
  alias: 'y',
  describe: 'Run without asking any questions (will still ask on ' + colors.red('--purge')[0] + ')'
}).usage('daisweb3 -y').help(
).option('confirm', {
  alias: 'c',
...
(async () => {
  ...
  process.on('SIGINT', (signal) => {
    const [purgingDirMsg, dirInRed] = colors.red(
      'purging directory',
      dir
    )
