...
;(async () => {
  ...
  // Write a template .daisconfig file
  if (argv.init)
  ...
  // Throw an error if unhyphenated args are defined
  // alongside the --assemble argument
  if (argv.assemble && argv._.length > 0) {
    const [text] = colors.red(
      'Cannot quick write and assemble at the same time'
    )
    log.withbox.error(`
    ${text}
    `)
  ...
  // Call quick write if unhyphenated args are defined
  if (argv._.length > 0)
    await QuickWrite(argv._, dir).catch(e => {
      log.error(e.message)
      process.exit(1)
    })
