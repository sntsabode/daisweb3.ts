...
import { mkdir, writeFile, readdirSync, rmSync, lstatSync } from 'fs'
...
export const trashDir = async (dir: string): Promise<void[]> => {
  return Promise.all(
    readdirSync(dir).map(async path_ => {
      const curPath = path.join(dir, path_)

      if (path_ === 'node_modules') return purgeDir(curPath)

      if (lstatSync(curPath).isDirectory()) {
        await trashDir(curPath).catch(e => {
          throw e
        })
      } else trash(curPath)
    })
...
export async function ask(lcb: () => void): Promise<'Y' | 'N'> {
  ...
  return new Promise((resolve, reject) => {
    ...
    read.question('Are you sure (Y/N)', a => {
      ...
      if (a === 'Y' || a === 'y' || a === 'yes') resolve('Y')
      if (a === 'N' || a === 'n' || a === 'no') resolve('N')
...
export const makeFile = async (path: string, data: string): Promise<void> =>
...
export const makeDir = async (path: string): Promise<void> =>
  new Promise((resolve, reject) =>
    mkdir(path, { recursive: true }, err => (err ? reject(err) : resolve()))
  )
...
export function log<L>(...msg: L[]): void {
  return console.log(...colors.cyan(tag), ...msg)
}

// eslint-disable-next-line
export namespace log {
  export const error = <E>(...msg: E[]): void =>
    console.error(...colors.red(tag), ...msg)

  export const success = <S>(...msg: S[]): void =>
    console.log(...colors.green(tag), ...msg)

  export const warning = <W>(...msg: W[]): void =>
    console.error(...colors.yellow(tag), ...msg)
  ...
  export namespace withbox {
    ...
    export const error = (templateLiteral: string, w = 50, h = 10): void =>
      console.error(
        new Box(
          {
            w,
    ...
    export const success = (templateLiteral: string, w = 50, h = 10): void =>
      console.log(
        new Box(
          {
            w,
